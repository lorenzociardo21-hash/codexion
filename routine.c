...
/*   Updated: 2026/06/15 11:55:09 by lciardo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	grab_both_dongles(t_coder *coder)
{
    size_t	time;
    size_t	left_release;
    size_t	right_release;

    take_both_dongles(coder);

    pthread_mutex_lock(&coder->left_dongle->lock);
    left_release = coder->left_dongle->last_release_time;
    pthread_mutex_unlock(&coder->left_dongle->lock);

    pthread_mutex_lock(&coder->right_dongle->lock);
    right_release = coder->right_dongle->last_release_time;
    pthread_mutex_unlock(&coder->right_dongle->lock);

    time = get_time() - left_release;
    if (time < coder->config->dongle_cooldown)
        ft_usleep((coder->config->dongle_cooldown - time), coder);
    time = get_time() - right_release;
    if (time < coder->config->dongle_cooldown)
        ft_usleep((coder->config->dongle_cooldown - time), coder);
    print_action(coder, "has taken a dongle");
    print_action(coder, "has taken a dongle");
